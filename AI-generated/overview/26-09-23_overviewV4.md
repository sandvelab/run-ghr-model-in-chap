...
**Version 4 — executive.** Supersedes [[26-09-23_overviewV3]], [[26-09-23_overviewV2]] and
[[26-09-22_overviewV1]], which are kept unchanged. Detail lives in the appendix at the foot;
every number here is read from a named file.

> Version 4 settles by demonstration what version 3 argued from source. A **third case** —
> `zlilu/minimal_template_example`, an `MLproject` template declaring a tunable `alpha` — was run
> through CHAP at three values. All three reached the model and changed the predictions. The
> project's own two models could not show this: route A's declares eighteen options that never
> arrive, route B's declares none.
...
A third case, `zlilu/minimal_template_example`, was run to settle what neither of these two could:
an `MLproject` template declaring one tunable option. **Its configuration arrived and changed the
predictions.**
...
## Configuration: three cases, two mechanisms

Configuration is where the two mechanisms genuinely differ, and the difference is not visible in
the effort counts at all.

| | Declares options | Can an operator set them through `chap eval`? |
|---|---|---|
| Route A — `chapkit_ghr_model` | **18** | **No.** Accepted, stored in an inert block, silently discarded |
...
differently — and this is the finding.** Case C was also run with the shape CHAP's own
`chap model schema --example` emits, a bare scalar where the model reads `alpha.values`:
...
**The structural point.** Route A depends on **8 distinct chapkit endpoints, called 198 times in
one evaluation**. Agreement is validated at `/api/v1/info`, where a mismatch fails loudly, and
not at `/api/v1/configs`, where it fails silently. An `MLproject` model has no negotiated payload
at all.
