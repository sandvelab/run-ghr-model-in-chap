# Provenance — minimalist_example_uv (route b)

One section per file. Append; never overwrite an existing section.

## repo/

- **What it is**: the model repository for route B — an MLproject-declared model run with uv, as published, with its `.git`
  directory removed so that the tree is versioned by *this* repository rather than being an
  invisible nested repository.
- **Source**: https://github.com/dhis2-chap/minimalist_example_uv
- **Pinned at commit**: `5cd8a1267362d945491353fba1c63a408462fa17` (see `git-metadata.txt` for the remote, the commit date and
  the subject line).
- **Obtained by**: `git clone <remote> repo && git -C repo checkout 5cd8a1267362d945491353fba1c63a408462fa17`, then
  `rm -rf repo/.git`, on 2026-09-21.
- **Licence**: **none declared** — no licence file, and GitHub reports no licence. Redistribution of this copy is not established; raised with the human before release.
