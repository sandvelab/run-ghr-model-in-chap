...
> **Start with the overview**: `AI-generated/overview/26-09-23_overviewV5.md`. It says what was
...
| Anchor | This project |
|---|---|
| Evaluation platform or harness | CHAP, `chap-core` 2.3.1 — resolving `chapkit` 2.1.0 and `servicekit` 2.0.2 — installed as a `uv` tool and invoked as `chap`. Archived metadata at `Archive/platform-chap/`. The resolved `chapkit` version is recorded alongside `chap-core`'s, because `chap-core` has not pinned it tightly in every release and a chapkit model service talks to `chapkit`, not to `chap-core`. What CHAP does with a model is the object of study and is not pre-specified. |
...
| Main environment | Pinned in batch 1: the Python that runs this project's own summarising and reporting scripts. Installed from `environment/lock.txt` by `environment/install-env.sh`; invoked as `environment/env/bin/python`. No Docker image for the analysis itself — but route A requires a container runtime, which is a property of the route and part of what is being measured. |
